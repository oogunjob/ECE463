...
int main(){
    ...
    while(1){
        ...
        // maintains connection from server to client
        else{
            if(fork() == 0){
...
void respond(int client_sock, struct sockaddr_in client, int database_sock, struct sockaddr_in database){
    ...
    char dst[INET_ADDRSTRLEN];
	  char clientMessage[99999], data_to_send[MAXLINE], path[MAXLINE]; // ask about what the max client message could be
    char* requestLine[3];
    ...
    // message was properly received and logs the client request to terminal
        ...
        // prints the client's request message
        fprintf(stdout, "\"");
        int i = 0;
        ...
        // if the first line of the request was a GET method, look for the file requested in web root
            ...
            // the protocol was successfully HTTP/1.0 or HTTP/1.1
			      else{
