...
void respond(int client_sock, struct sockaddr_in client, int database_sock, struct sockaddr_in database){
  ...
  // message was properly received and logs the client request to terminal
    ...
    // if the first line of the request was a GET method, look for the file requested in web root
      ...
      fprintf(stdout, "real request: %s\n", requestLine[1]);
      requestLine[1] = "cutecat"; // need to remove just for debugging purposes
      fprintf(stdout, "new request: %s\n", requestLine[1]);
      ...
      // if the protocol is supported and it is not a bad request, check if the server can carry out request
        ...
        // checks if the file path is a request to search the data base
          ...
          char buffer[MAXLINE]; // buffer that stores database's response

          // retrieve the database's response and store the number of bytes read
          bytes_read = recvfrom(database_sock, (char*)buffer, MAXLINE, 0, (struct sockaddr*)&database, &len);

          // if the database could not retrieve the file, send a 404 Not Found response back to client
          ...
          // if the data successfully found the file, write the conents of the file back to the client
          else{
